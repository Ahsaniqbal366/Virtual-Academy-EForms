...
// define component
@Component({
    selector: 'sls-message-center-archive',
    templateUrl: 'sls-message-center-archive.html',
    styleUrls: ['sls-message-center-archive.scss'],

})

// create class for export
export class MessageCenterArchiveComponent implements OnInit {
    ...
    ********************************************/
    ...
    private _setupDefaultProperties(): void {
        ...
        this.selectedTab = MessageCenterTab.Archive;
    }

    // Called from [_init()], sets, if needed, [messageCenterProvider.archivedMessages]
    private async _getArchivedMessages(): Promise<void> {
        ...
        return await new Promise(async (resolve, reject) => {
            ...
            this.messageCenterProvider.getActiveMessages(senderOrRecipientBool, isArchived).subscribe(
                (archivedMessages: any) => {
                    this.messageCenterProvider.archivedMessages = archivedMessages || [];
                    // Set GUIData on each
                    this.messageCenterProvider.archivedMessages.forEach(loopedArchivedMessage => {
                        loopedArchivedMessage.GUIData = {
                            isChecked: false
                        } as MessageGUIData;
                    });
                    resolve();
    ...
    *******************************************/
    private async _init() {
        ...
        await this._getArchivedMessages();
        await this.messageCenterProvider.updateUnreadCounts();
        this.messageCenterProvider.headerText = 'Archive';
        this.messageCenterProvider.subView = 'archive';
